...
PendingOrder::PendingOrder(const ActionType &action_type,
    ...
    : env(Environment::get_instance()),
      ...
      cur_price_when_generated(env->feeds[ticker]->execute_price()),
...
PendingOrder::PendingOrder(const ActionType &action_type,
    ...
    : env(Environment::get_instance()),
      ...
      cur_price_when_generated(signal->execute_price),
      difference(set_difference()){};
...
const double PendingOrder::set_difference() {
    if (trigger_key.find("pct") == string::npos)
        return signal_info[trigger_key] / size;
    return signal_info[trigger_key] * cur_price_when_generated;
};
...
const double PendingOrder::below_price(const double diff) {
    return cur_price_when_generated - diff;
};
...
const double PendingOrder::get_cur_price_when_generated() const {
    return cur_price_when_generated;
};
